...
| 備料吸乾與燒帳修正（v1.8.3）| 兩個互相放大的帳務 bug：①**總成滿補無界**（v1.3.6）把單一料的全網存量抽進一顆 CPU（實錄液態氦 6.5 億 mB＋氦氣 3.75 億 mB 囤在銩錠單），其他要同料的單全部餓死 → 放寬改有界（cap×8=65536 輪）；②**自庫認領超額判準用 capped 需求**（8192 輪）對不上無界塞入的備料——99% 工作備料被誤判為「繞過認領的產出」，每秒拿去燒 `waitingFor` 帳（實錄每秒 3800 萬→1 億遞增）→ `remainingDemand` 改全量不截斷。啟動橫幅版本改讀 ModList 動態值（先前寫死 v1.8.0 誤導法醫）。~~本次考證同時認定「提前收單皆數量足額」~~——**此結論錯誤，被 v1.8.4 推翻**（不吃保母成品餵料的單（如鋅碘陰極，繞過認領直進 CPU 庫存）確實足額；吃餵料的 standalone 單被雙重銷帳短交，見下列）|
| 玩家單成品餵料雙重銷帳（v1.8.4）| **實錄：玩家單 100k 桶不純超能硅岩溶液做到 50k 就收單**——standalone（玩家終端）單的 `link.insert` 恆收 0：GTO 認領攔截對每批真交付**燒帳後貨落回網路**（此即 standalone 單的正常送達機制，貨在 ME＝已交付）；保母把這些落回的貨當成「待認領盈餘」再餵回去**又燒一次帳**（基線防偽擋不住——它們確實是開單後新增），每桶貨燒兩次 → 帳在實產 50% 時燒完收單。修法：**standalone 單成品一律不餵**（`ICraftingLink.isStandalone()` 公開 API；判不出保守不餵），requester 單照餵（其 link 實收貨、無 fallthrough）。中間產物餵料不受影響（認領攔截把中間產物收進 CPU 庫存、無 fallthrough、無雙燒） |
