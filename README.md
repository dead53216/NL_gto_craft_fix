...
- **2.0.0 ＝ 原始碼全面退回 1.1.0**（使用者決定：1.1.x–1.8.x 的加速/救援/通知系統反覆引發新問題——雙重銷帳、提前收單、吸乾備料、訊息洗版——全數移除，回到可信基線；期間的考證與教訓見 git 歷史與各版 commit）。啟動橫幅印 `v1.1.0` ＝ 行為版本，屬預期。
...
| 並行死角解鎖（2.1.0）| **上游 bug**（`OptimizedCraftingCpuLogic.executeCrafting:221-238`，已回報）：並行分支漏了 `parallel==1` 的取料路徑——「並行樣板＋剩餘輪數>1＋庫存恰夠 1 輪」被每 tick 無聲跳過、永久卡死。催化劑返還配方（吃 9216 還 4608 錫鐵合金的中子反射板實錄）按淨需求備料必然踩中，小量下單 100% 重現、與算料器無關。修法：保母命中指紋（min⌊庫存/每輪⌋==1 且剩>1 輪）時把輸入補到 2 輪份，讓 GTO 自己的 `parallel>1` 分支正常取料——只補料，不代推送不碰帳 |
