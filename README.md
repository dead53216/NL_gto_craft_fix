...
| 深鏈凍結死枝豁免（v1.8.5）| v1.8.2 的「本單在製中就凍結斷料點名」有盲區：**缺的料這張單自己也沒有任何剩餘輪次會產**（計畫把需求記在網路現貨上、沒排生產）且全網零在途＝死枝，永遠沒人會補——實錄 10 份基岩採礦機訂單缺 `hv_digital_miner`（每輪 4、全網 0、零在途）整單卡死，卻因 HV 馬達等其他分支在製而被凍結靜默。修法：凍結加第三條件 `jobProduces(ik)`——死枝照樣 60 秒點名 |
