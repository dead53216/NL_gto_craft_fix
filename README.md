...
| 完單短交補單（v1.6.0）| GTO 執行器**預測性收單**（任務全派出即 finishJob，帳未清照收——hssg 兩張 20000 單各短交 1568 實錄）沒辦法直接擋：fork 把 `CraftingCpuLogic` 挖成抽象殼、`finishJob` 在 gtolib 閉源子類（mixin 無聲失效）。改從結果端補正：完單法醫抓到「job 消失、link 未取消、交付帳剩 N、在途 M」的瞬間，淨短交 N−M > 0 → 走救援管線自動補一張差額頂層單＋聊天室「完單短交：帳差 N，已啟動自動補產」。**只補非訂單 job**（訂單型維持 GTO 收據制——每張皆帳未清收單、貨後補落庫，擋了週期訂單全變殭屍）；撤單死法不補（尊重玩家取消） |
