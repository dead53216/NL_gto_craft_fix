...
| 斷料救援自動下單（v1.5.0）| 斷料點名觸發的同時，若缺料**有樣板**→ 自動下一張涵蓋全部剩餘需求（每輪需求×剩餘輪數）的**頂層**補產單，走正規機器源管線（lpcalc→計畫修補→present-once IgnoreMissing），產出落網路後保母自動餵入凍結單解凍；聊天室回報「已啟動自動補產 xN」。這是鐵則「禁止巢狀合成請求」的唯一豁免（當年災難＝每缺口每秒代下碎單佔滿 CPU），五道閘：60 秒斷料才觸發、一 key 一張整單、**深度 1**（救援單自己斷料不再往下開）、算料中＋在途合計上限 4、per-key 冷卻（成功 10 分鐘/失敗 2 分鐘）。無樣板仍走提示請玩家補料 |
| 完單時點復原（v1.7.0）| 提前收單的**製造者其實是本 mod 自己的加速**：1.1.0 小額慢補時最終輪次被產能自然節流、收單≈真完單；1.1.6「補輸入改全額」起（1.2.0 即現「下單十份剩四份」，時間點吻合）→ 1.3.4 吞吐加倍 → 1.3.6 總成滿補，最終產物輪次數秒內全數派進總成，GTO「全部派完即 finishJob」→ 開跑沒多久就關帳。修法：**只對「主產物＝job 最終產物」的任務**恢復節流——在途 ≥ P 輪（`gtodiag.lpcalc.finalInflightRounds`，預設 64，0＝停用）暫停補其輸入、產出回流沖帳再續補，最後一輪派出時帳已幾乎清空、**收單時點回到≈真完單**；中間產物任務維持滿補（吞吐不變）。訂單 job 與帳面異常（在途輪＞剩餘輪＝waitingFor 預填）不節流。節流暫停時不進缺料/斷料判定（防誤報誤救） |
| 完單短交監看（v1.6.0，v1.6.1 認知修正）| GTO 執行器**預測性收單**（單據全數推入機器即 finishJob，帳未清照收）攔不到（fork 把 `CraftingCpuLogic` 挖成抽象殼、`finishJob` 在 gtolib 閉源子類，mixin 無聲失效）、也**不該立即補差額**——帳差多半是「機器還在做」，成品稍後自行落庫，CPU 抱著的成品完單時也會 storeItems 退庫（v1.6.0 立即補＝重複生產；鈦/鎵類完單常態帳剩 55 萬/911 萬全是名存實亡的死帳）。v1.6.1 改三道閘後只補真損失：①**只看玩家單**（submitJob 成功時標記 link；機器源有 requester 水位制自我修復）②完單瞬間**先扣網路現貨**抵帳（死帳直接歸零）③剩餘「應到未到」量**監看 5 分鐘**（cachedInventory 每 tick 正差分累計到貨），到帳蓋過即結案零動作；期滿仍未到的餘額才走救援管線補產＋聊天室「完單短交確認：實際少 N」。訂單型維持 GTO 收據制；撤單不看（尊重玩家取消）。提前收單「以前看不到」的原因：多步大單過去根本卡死、到不了「全部派完」，本 mod 修順補料後大單數十秒派完，該行為才天天可見（完單法醫 v1.2.0 起即有記錄） |
...
- **一鍵停用**：`-Dgtodiag.lpcalc.enabled=false` → 機器路徑完全走現行樹狀版（預設 true）。
  ...
  `gtodiag.lpcalc.topUpRoundsCap`（保母補輸入單次最多補幾輪份量，預設 8192——防長單把網路庫存整鍋吸進單一 CPU；**供應器為樣板總成系列（PatternBuffer，無限槽）時無視 cap 一次補滿全部剩餘輪**）、
  `gtodiag.lpcalc.finalInflightRounds`（最終產物任務在途輪深度節流，預設 64、0＝停用——完單時點復原，見上表 v1.7.0）。
