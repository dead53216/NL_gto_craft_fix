...
| 斷料點名防誤報（v1.8.1）| 「合成時間久」≠卡住：缺料項若**全網有在途生產**（GTO 版 `getRequestedAmount`＝各 CPU `waitingFor` 合計，涵蓋本單慢機器與玩家另開的補料單；API 例外退看本單）→ 凍結 60 秒斷料計時、不點名。只有「缺料＋網路乾＋零在途」連續 60 秒才通知（訊息加註「期間無任何機器在製」）；「任務缺料」WARN 補印在途量，log 可分辨慢/卡 |
