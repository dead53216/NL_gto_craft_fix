...
- mixin 原則掛 AE2 類（`appeng.*`）；GTOCore/gtolib 類優先用反射。（當年「mixin 無聲失效」真因是 AE2-gto fork 把 `CraftingCpuLogic` 挖成純抽象殼、目標方法體不存在。）**既有例外（2.2.0，使用者核准）**：`OptimizedCpuParallelFixMixin` 掛 GTOCore 開源類根治上游 parallel==1 死角——`targets` 字串綁定不引 GTOCore 編譯依賴、反射欄位失配即自動停用；新增 GTOCore 類 mixin 仍須使用者逐案核准。datasynclib holder 編譯類在 `1.20.1/forge/libs/`（自整合包抽出，升級整合包後如欄位變動需重抽）。
